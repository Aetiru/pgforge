...
**Paleta de comandos ()**: acciones, vistas, pestañas abiertas, servidores y los objetos que el árbol **ya trajo**. No consulta nada al servidor —tiene que responder en el mismo cuadro, y el catálogo de una base grande no entra en esa promesa—; para lo que el árbol no cargó está su búsqueda, que sí pregunta. La coincidencia es por subsecuencia y sin acentos (, puro y con Vitest), porque uno escribe tanto las iniciales («nc» para «Nueva consulta») como el pedazo del medio de un nombre; a igual coincidencia manda el tipo de comando, y con la caja vacía no se ordena nada. Cierra con clic afuera, al revés que : no hay nada escrito que perder, y una paleta que hay que cerrar con un botón deja de ser más rápida que el camino largo.
