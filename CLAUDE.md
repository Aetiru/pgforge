...
Leer el catálogo y decidir están separados a propósito: `index_shapes` trae la forma de cada índice y `redundancies` es **pura**, porque equivocarse acá significa proponer que se borre un índice que hace falta. Lo que nunca se propone: un índice único o de clave primaria porque otro más largo lo empiece —sostiene una restricción, no una consulta—, uno con `INCLUDE` a favor de uno que no lo tiene, y cualquier par con distinto método o distinto predicado, que no responden lo mismo. Con columnas idénticas se conserva uno solo y el desempate está escrito (restricción, `INCLUDE`, uso, nombre): decidirlo dos veces —una por cada orden del par— dejaría la tabla sin ninguno.

Las columnas salen de `pg_get_indexdef(indexrelid, n, true)`, columna por columna, y no de `pg_attribute`: así un índice por expresión se lee igual que uno por columna. El test contra servidores reales (`tests/monitor_indices.rs`) arma el caso completo —dos iguales, un prefijo, un parcial, un gin y un único— y verifica en las cinco versiones que solo aparezcan los dos que sobran.
